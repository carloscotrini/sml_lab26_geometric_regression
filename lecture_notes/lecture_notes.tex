...
The angle $\theta$ between $\tilde{\by}$ and $\tilde{\yhat}$ satisfies
\[
  \cos\theta = \frac{\norm{\tilde{\yhat}}}{\norm{\tilde{\by}}},
\]
so
...
and its projection.
A small $\theta$ (data close to its shadow) gives $R^2$ near 1.
A large $\theta$ (data far from its shadow) gives $R^2$ near 0.
\end{keyinsight}
...
In a multiple regression $\by = \bX_1\bm{\beta}_1 + \bX_2\bm{\beta}_2 + \be$,
the coefficient $\bhat_2$ answers: \emph{what is the effect of $\bX_2$
after removing the influence of $\bX_1$?}
...
When predictors $\bX_1$ and $\bX_2$ are correlated,
$\bM_1\bX_2$---the residualised predictor---is \emph{shorter}
than $\bX_2$.
The unique information in $\bX_2$ (after removing $\bX_1$) shrinks.
...
As correlation increases, $\norm{\bM_1\bm{x}_{2,j}}\to 0$ and
$\text{SE}\to\infty$.
This is the geometric root of \emph{multicollinearity}.
...
An observation with a moderate leverage but a large residual at an
unusual predictor combination can quietly torque the coefficients.
\end{keyinsight}
...
The eigenvalue decomposition of $\bX^\top\bX$ reveals this:
\[
